...
class GranularBall:
    ...
    def split_2balls(self):
        ...
        clu=KMeans(n_clusters=2,random_state=SEED).fit(self.data_no_label)
...
class GBList:
    ...
    def re_division(self, i):
        ...
        clu = KMeans(n_clusters=k, init=self.get_center()[:, attributes], max_iter=1, random_state=SEED).fit(self.data[:, attributes])
...
def generate_ball_data(data,pur,delbals):
    ...
    for i in range(len(gb.granular_balls)):
        ...
        if gb.granular_balls[i].label==-1:
            a.append(-1)
        elif gb.granular_balls[i].label==0:
            a.append(0)
